...
public class WaitForPort {
	...
	private static final Logger logger = LoggerFactory.getLogger(WaitForPort.class);
	private final int waitTimeInMilis;
	private final int maxIteration;

	/**
	 * Waits for services in format "host1:port1;host2:port2;..."
	 *
	 * It waits with default values of 1000 ms between checks and max number of checks per host is 1000.
	 *
	 * @param hostWithPorts
	 *            string that represents hosts and ports
	 */
	public static void waitForServices(String hostWithPorts) {
		if (hostWithPorts == null) {
			logger.info("No service to wait for. Continuing immediately.");
	 ...
	 *
	 * @param waitTimeInMilis
	 *            time beetween checks
	 * @param maxIteration
	 *            maximal number of iteretion per pair of host and port
	 */
	 ...
	 *
	 * @param hostsWithPorts
	 *            this format is following "host1:port1;host2:port2;..."
	 *
	 * @throws RuntimeException
	 *             when max wait time for some host is reached.
	 */
	public void start(String hostsWithPorts) {
		List<HostPort> hostPorts = parse(hostsWithPorts);

		for (HostPort hp : hostPorts) {
			logger.info("Checking availability of service: " + hp);
			for (int i = 1; i <= maxIteration; i++) {
				try (Socket socket = new Socket(hp.host, hp.port)) {
					break;
				} catch (IOException e) {
					logger.info("Waiting for " + hp);
					try {
				...
				if (i == maxIteration) {
					throw new RuntimeException("Max wait time reached for service " + hp);
				}
			}
		}
	 ...
	 *
	 * @param hostsWithPorts
	 *            string in format: "host1:port1;host2:port2;..."
	...
	/**
	 * Try to find property in environment variables. If there is no property it tries in properties (-D option on startup).
	 * 
	 * @param propertyName
